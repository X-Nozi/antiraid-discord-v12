...
module.exports = class {
    ...
    async run(member) {
        ...
        if (member.user.bot) {
            ...
            const action = await member.guild.fetchAuditLogs({ type: "BOT_ADD" }).then(async (audit) => audit.entries.first());
            ...
            if (client.config.owners.includes(action.executor.id) || guild.owner.id == action.executor.id || guildData.whitelist.includes(action.executor.id) || !guildData.antibot) {
                let logChannel = client.guilds.cache.get(guild.id).channels.cache.get(guildData.protectLog)

                if (logChannel) logChannel.send(client.translator(guildData.language).event("BOT_ADD", {
                    ...action,
                    timeout: Date.now() - startAt,
                    triggered: false
                }))
            } else if (guildData.antibot && !client.config.owners.includes(action.executor.id) || guild.owner.id !== action.executor.id) {
                member.kick(`Protection - Type: ${this.name}`)
